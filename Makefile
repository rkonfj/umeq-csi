...
all: agent plugin
plugin:
	sudo nerdctl build --build-arg VERSION=${version}-${git_hash} . -t ${oci_registry}/umeq-csi:${version};\
	sudo nerdctl push ${oci_registry}/umeq-csi:${version}
agent:
	cd cmd/agent;\
...
dist:
	sudo nerdctl run --rm -e GOPROXY=https://goproxy.cn -v ${shell echo $GOPATH}:/go \
